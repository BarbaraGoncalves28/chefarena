  ...
  return (
    <section className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between gap-4">
        ...
        <button onClick={refresh} className="rounded-md border border-zinc-300 px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-100">
          Refresh
        </button>
      </div>
      ...
      {!seasonId ? (
        ...
        <div className="mt-6 overflow-x-auto">
          <table className="min-w-full text-left text-sm text-zinc-700">
            ...
            <tbody>
              {(ranking.ranking?.rows ?? ranking.ranking?.contestants ?? []).map((row) => (
                <tr key={row.contestantId} className="odd:bg-zinc-50">
                  <td className="border-b px-3 py-3 font-medium">{row.position ?? row.rank}</td>
                  <td className="border-b px-3 py-3">{row.name}</td>
                  <td className="border-b px-3 py-3">{(row.rankingScore ?? row.effectiveScore ?? 0).toFixed(1)}</td>
                  <td className="border-b px-3 py-3">{row.immunity || Number(row.immunities ?? 0) > 0 ? "Yes" : "No"}</td>
